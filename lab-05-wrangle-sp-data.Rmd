...
nrow(dn_ak)
```
***There are 3 locations in Alaska***
...
nit, *commit, and push your changes to GitHub with an appropriate commit message. Make sure to commit and push all changed files so that your Git pane is cleared up afterwards.*
...
dn_lq_ak <- dn_lq_ak %>%
  mutate(
    distance_km = haversine(longitude.x, latitude.x, longitude.y, latitude.y)
  )
```
...
```{r}
nearest_distances <- dn_lq_ak %>%
  group_by(longitude.x) %>%  
  summarize(min_distance = min(distance))
```
```{r}
ggplot(nearest_distances, aes(x = min_distance)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Distances to Nearest La Quinta from Denny's",
...
```{r}
dn_nc <- dn_ak %>% filter(state == "NC")
lq_nc <- lq_ak %>% filter(state == "NC")
...
nearest_distances_nc <- dn_lq_nc %>%
  group_by(address.x) %>% 
  summarize(min_distance = min(distance_km))


summary_statistics_nc <- nearest_distances_nc %>%
  summarize(
...
print(summary_statistics_nc)


library(ggplot2)
ggplot(nearest_distances_nc, aes(x = min_distance)) +
  geom_histogram(binwidth = 10, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Minimum Distances Between Denny's and La Quinta Locations in NC",
       x = "Distance (km)", y = "Frequency") +
  theme_minimal()
```
10. Repeat the same analysis for Texas.
```{r}
dn_tx <- dn_ak %>% filter(state == "TX")
lq_tx <- lq_ak %>% filter(state == "TX")
...
nearest_distances_tx <- dn_lq_tx %>%
  group_by(address.x) %>%  
  summarize(min_distance = min(distance_km, na.rm = TRUE)) 




summary_statistics_tx <- nearest_distances_tx %>%
  summarize(
    mean_distance = mean(min_distance, na.rm = TRUE),
    median_distance = median(min_distance, na.rm = TRUE),
    sd_distance = sd(min_distance, na.rm = TRUE),
    min_distance = min(min_distance, na.rm = TRUE),
    max_distance = max(min_distance, na.rm = TRUE)
  )


print(summary_statistics_tx)


ggplot(nearest_distances_tx, aes(x = min_distance)) +
  geom_histogram(binwidth = 10, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Minimum Distances Between Denny's and La Quinta Locations in TX",
       x = "Distance (km)", y = "Frequency") +
  theme_minimal()
```
...
```{r}
dn_ny <- dn_ak %>% filter(state == "NY")
lq_ny <- lq_ak %>% filter(state == "NY")


dn_ny <- full_join(dn_ny, lq_ny, by = "state")
...
nearest_distances_ny <- dn_ny %>%
  group_by(address.x) %>%  
  summarize(min_distance = min(distance_km, na.rm = TRUE))  # Minimum distance to nearest La Quinta


summary_statistics_ny <- nearest_distances_ny %>%
  summarize(
    mean_distance = mean(min_distance, na.rm = TRUE),
    median_distance = median(min_distance, na.rm = TRUE),
    sd_distance = sd(min_distance, na.rm = TRUE),
...
print(summary_statistics_ny)


ggplot(nearest_distances_ny, aes(x = min_distance)) +
  geom_histogram(binwidth = 10, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Distribution of Minimum Distances Between Denny's and La Quinta Locations in NY",
       x = "Distance (km)", y = "Frequency") +
  theme_minimal()
```
...
***Alaska would most likely fit the joke the most due to the sparse population of Alaska and the distances between locations. Alaska is significantly less dense than the other states mentioned***
